...
const report = {
  schema_version: 1, purpose: 'p02_catalog_development_validation', profile,
  ...
  definitions: {
    ...
    context_ready_ms: 'Reserved for P06 vector-layer readiness; unavailable during P02 catalog-only delivery',
    ...
    catalog: 'P02 compact persisted metadata catalog; environmental geometry delivery remains unavailable until P06', tiles: 'Not implemented',
  ...
  catalog_ready_ms: null, tile_bytes: null, usable_context_complete: false,
  cold: [], interactions: [], api: [], errors: [], complete: false
};
...
async function coldLoad(browser, index) {
  ...
  try {
    ...
    const catalogControl = await until(async () => {
      const preparing = await page.getByText("Tiles are being prepared for this layer.").count();
      const disabled = await page.locator('.layer-list input[type="checkbox"]:disabled').count();
      return preparing > 0 && disabled > 0;
    }, 'catalog layers visible as preparing', 180_000);
    const marks = await page.evaluate(() => Object.fromEntries(performance.getEntriesByType('mark').map((mark) => [mark.name, mark.startTime])));
    for (const required of ['p01:records-received', 'p01:list-ready', 'p01:map-loaded']) {
      if (!Number.isFinite(marks[required])) throw new Error(`Missing required mark: ${required}`);
    }
    await network.flush();
    Object.assign(result, {
      marks, feature_rendered_ms: dev.time, catalog_preparing: catalogControl,
      first_useful_ms: dev.visible ? Math.max(dev.time, marks['p01:list-ready'], marks['p01:records-received']) : null,
      context_ready_ms: null,
      scroll_required: !dev.visible, initial_development: dev,
    ...
    if (result.network.some((row) => new URL(row.url).pathname === '/api/environmental-overlays')) throw new Error('Legacy environmental bulk endpoint was requested');
...
try {
  ...
  for (const endpoint of ['/api/development-records', '/api/map/layers']) {
