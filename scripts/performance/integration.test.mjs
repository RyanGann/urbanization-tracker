...
function stubRun(statsOutput, statsCode = 0) {
  return async (_command, args) => {
    ...
    if (args[0] === 'inspect') {
      return { code: 0, output: args.includes('HostConfig') ? '{"Memory":1073741824,"NanoCpus":1000000000}' : '{}' };
    }
...
test('resource sampling succeeds with nonempty API and DB stats rows', async () => {
  ...
  const stats = [
    JSON.stringify({ ID: 'api-container', Name: 'api', CPUPerc: '1.00%' }),
    JSON.stringify({ ID: 'db-container', Name: 'db', CPUPerc: '2.00%' })
  ].join('\n') + '\n';
  const stopSampling = await startResourceSampling({
    compose: ['compose', '--project-name', 'p01-test'],
    run: stubRun(stats),
