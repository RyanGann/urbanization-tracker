...
Only completed, cleaned synthetic A/B runs with matching fixture hashes can be archived. The helper copies selected JSON evidence and removes local absolute paths and unrelated container names. Snapshot data, generated full geometry, logs, environment files and screenshots remain local. Existing archive names are never overwritten. The final resources file includes every raw sample, so its redundant partial checkpoint is not archived.
...
Eleven affected measurement tests pass, including streamed gzip byte counts, timeout failure, mandatory resource sampling, conventional medians, and awaiting delayed worker size accounting. Worker response transfer uses completed Playwright encoded body plus response header sizes when page-target CDP omits completion, with explicit provenance. Unknown decoded body sizes remain null. The [Playwright request size contract](https://playwright.dev/docs/api/class-request#request-sizes) defines those fields. Older archived smoke medians used nearest-rank p50; their raw samples are preserved, and no two-sample latency budget is claimed.

The combined real-stack functional run at `1e074dd3894bc5bff53c58a2e4ce06e9c495a5f2` passed two fresh loads, two complete warm cycles, and eight API attempts; cleanup removed. Raw artifact: `tmp/integration/2026-09-20T03-55-39-085Z-2e7583d6`. This validates C01 integration and the awaited worker fallback.

That real run also exposed a Chromium limitation: Playwright reported a completed, visibly working worker script with zero encoded body bytes. Those zero values cannot establish that the worker was free to download. The reporter now preserves the raw counter separately, emits null worker body/total transfer with `worker_body_counter_unavailable`, and leaves whole-bundle transfer unknown. A regression covers the observed zero-counter case. Older archived network aggregates omit the unavailable worker body and must not be used as complete bundle-size evidence; complete response byte accounting for the regular application APIs remains available. This is an explicit measurement limitation for future P09 work, not a passed bundle budget.

Required CI includes the performance functional precheck, T01 real API/browser smoke, and C01 data-mode scenario. The [implementation PR](https://github.com/RyanGann/urbanization-tracker/pull/17) records the final-head check and Codex review results.
