...
| Demo catalog IDs `pilot-boundary`, `wetlands`, `floodplain`, `hydrography`, `parks-open-space` | No tile URL is configured, but demo `/api/environmental-overlays` exposes seed geometry. Do not treat demo status as rights clearance. | Establish whether each seed feature was independently authored or derived from its linked City/FEMA/USFWS service; approve exact displayed bytes, attribution, retention and any future tile/export use per version. Hydrography and parks/open-space need their own producer/terms review. |
| Demo development records in `seed_data.json` | Four seeded records and their geometry are already public through demo development/GeoJSON routes; the staged agenda example remains reviewer-only. Do not infer seed permission from the matching live-feed rows. | Record seed file checksum/deployment commit; establish title/field/geometry lineage and source-use/privacy approval for each of the four public records. Review the staged agenda item separately before publication. Gate unresolved demo records before a public demo release. |
| Deployment-specific legacy overlay IDs | Treat every stored ID projected by compatibility backfill as unresolved until inventoried; no blanket approval based on the configured feed list. | Enumerate the actual live catalog and overlay arrays before release, record source URL/version/checksum and per-use owner decision for every ID, then verify unknown IDs are withheld on catalog, geometry, tile and export paths. |
