...
The rebased tree also passed `ruff check app tests`, `mypy app` (45 modules) and the full installed API suite (171 tests, two upstream TestClient deprecation warnings) in the pinned local check image. PR review and CI remain to be completed on the final documentation head. The live run proves the runtime at `db7a488`, not deployment readiness.
