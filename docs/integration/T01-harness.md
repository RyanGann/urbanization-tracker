...
beside the evidence. The runner otherwise verifies resource labels before removing its own
containers, networks and volumes. Future suites or scenarios return a clear nonzero
...
| `node scripts/run-integration.mjs --suite api --assert-failure` | failed as designed with `deliberate HTTP assertion failure`; cleanup passed | `2026-09-20T01-37-47-972Z-315bebe3` |
